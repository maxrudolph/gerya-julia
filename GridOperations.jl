...
function compute_strainrate_cylindrical(grid::CartesianGrid,vx::Matrix{Float64},vy::Matrix{Float64})
    # Inputs:
    # grid
    # vx and vy are velocities at the velocity nodes
    # Outputs:
...
function compute_stress_cylindrical(grid::CartesianGrid,vx::Matrix{Float64},vy::Matrix{Float64},etan::Matrix{Float64},etas::Matrix{Float64},etax::Matrix{Float64})
    # compute stresses.
    # Inputs:
    # grid
    # vx and vy at the velocity nodes
    # etan is viscosity at the cell centers
    # etas is viscosity at the basic nodes
    # etax is viscosity at vx nodes
    # returns:
    # sxx and syy at the cell centers
    # sxy at the basic nodes
    # stt at vx nodes
    exx,eyy,exy,ett = compute_strainrate_cylindrical(grid,vx,vy)
    sxy = 2.0*etas .* exy
    sxx = 2.0*etan .* exx
    syy = 2.0*etan .* eyy
    stt = 2.0*etax .* ett
...
function compute_shear_heating_cylindrical(grid::CartesianGrid,vx::Matrix{Float64},vy::Matrix{Float64},etan::Matrix{Float64},etas::Matrix{Float64},etax::Matrix{Float64})
    # compute shear heating at the cell centers
    # inputs: 
    # grid - the Cartesian grid
    # vx, vy - velocity at the velocity nodes
    ...
    exx,eyy,exy,ett = compute_strainrate_cylindrical(grid,vx,vy)
    # 2. compute stress components
    sxx,syy,sxy,stt = compute_stress_cylindrical(grid,vx,vy,etan,etas,etax)
    # 3. average sxy*exy over each cell
    sxyexy = zeros(grid.ny+1,grid.nx+1)
    sttett = zeros(grid.ny+1,grid.nx+1)
    for i in 2:grid.ny
        for j in 2:grid.nx
            sxyexy[i,j] = 0.25*(sxy[i,j]*exy[i,j]+sxy[i-1,j]*exy[i-1,j]+sxy[i,j-1]*exy[i,j-1]+sxy[i-1,j-1]*exy[i-1,j-1])
            sttett[i,j] = 0.5*(stt[i,j-1]*ett[i,j-1]+stt[i,j]*ett[i,j])
        end
    end
    # 4. compute shear heating
    shear_heating = sxx .* exx + syy .* eyy + 2.0*sxyexy + sttett
