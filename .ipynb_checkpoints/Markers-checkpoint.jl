...
function effective_viscosity!(m::Markers,maxeta::Float64,mineta::Float64)
    EII = m.scalarFields["EII"]
    SII = m.scalarFields["SII"]
    exx = m.scalarFields["exx"]
    eyy = m.scalarFields["eyy"]
    exy = m.scalarFields["exy"]
    sxx = m.scalarFields["sxx"]
    syy = m.scalarFields["syy"]
    sxy = m.scalarFields["sxy"]
    eta = m.scalarFields["eta"]
    T = m.scalarFields["T"]
    P = m.scalarFields["P"]
    Ea = m.scalarFields["Ea"]
    Threads.@threads for i in 1:m.nmark
        # second strain rate invariant & stress invariant
        m.scalars[EII,i]=(m.scalars[exx,i]^2+m.scalars[exy,i]^2)^0.5
        m.scalars[SII,i]=(m.scalars[sxx,i]^2+m.scalars[sxy,i]^2)^0.5
        # Compute effective strain rate invariant
        SS=0.925;
        m.scalars[EII,i]=m.scalars[EII,i]^(1-SS)*(m.scalars[SII,i]/2/m.scalars[eta,i])^SS;
        # recompute eta on marker
        m.scalars[eta,i] = viscosity(m.scalars[EII,i],m.x[2,i],m.scalars[T,i],m.scalars[Ea,i],m.scalars[T,i])
        if m.scalars[eta,i]>maxeta
           m.scalars[eta,i] = maxeta
        elseif m.scalars[eta,i]<mineta
           m.scalars[eta,i] = mineta
        end
    end
end